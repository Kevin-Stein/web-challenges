...
const lastNameFind = employees.find((employee) =>
...
function findById(employees, id) {
  const findId = employees.find((employee) =>
    employee.id === id)
  return findId;
}
...
function findByCitySubString(employees, string) {
  const city = employees.find((employee) =>
    employee.city.toLowerCase() === string.toLowerCase())
...
function findByProfessionSubStringAndAge(employees, string, age) {
  const professionAndAge = employees.find((employee) =>
    employee.profession.includes(string) && employee.age >= age)
  
  return professionAndAge;
}
